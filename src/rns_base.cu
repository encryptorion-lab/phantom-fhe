...
// T = double or uint64_t
    __global__ void
    decompose_uint64(std::uint64_t *dst, const T coefft, const bool is_negative, const DModulus *modulus,
    ...
    __global__ void decompose_uint128(std::uint64_t *dst, const double coeffd, const bool is_negative,
                                      const DModulus *modulus, const uint32_t poly_degree,
    ...
    __global__ void
    decompose_uint(uint64_t *dst, const uint64_t *coeffu, const bool is_negative, const DModulus *modulus,
                   const uint32_t poly_degree, const uint32_t coeff_mod_size) {
        for (size_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < poly_degree * coeff_mod_size;
             tid += blockDim.x * gridDim.x) {
            size_t twr = tid / poly_degree;
    ...
    void DRNSBase::decompose_array(uint64_t *dst, const cuDoubleComplex *src, const uint32_t sparse_poly_degree,
        ...
        if (max_coeff_bit_count <= 64) {
            decompose_array_uint64<<<gridDimGlb, blockDimGlb>>>(dst, src, base(), sparse_poly_degree, sparse_ratio,
                                                                size());
            ...
            decompose_array_uint_slow_second_part<<<gridDimGlb, blockDimGlb>>>(dst, coeffu.get(), base(),
                                                                               sparse_poly_degree, sparse_ratio,
                                                                               size());
        }
    }
    ...
    void DRNSBase::compose_array(cuDoubleComplex *dst, const uint64_t *src, const uint64_t *upper_half_threshold,
        ...
        compose_kernel<<<gridDimGlb, blockDimGlb>>>(dst, temp_prod_array.get(), acc_mod_array.get(), src, size(),
                                                    base(),
 ...
 */
    ...
    void
    DRNSBase::decompose_array(uint64_t *dst, const uint64_t *src, const DModulus *modulus, const size_t poly_degree,
