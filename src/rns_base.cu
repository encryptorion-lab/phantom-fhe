...
__global__ void decompose_array_uint64_kernel(uint64_t *dst, const cuDoubleComplex *src, const DModulus *modulus,
                                              const uint32_t poly_degree) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    size_t twr = tid / poly_degree;
    size_t coeff_id = tid % poly_degree;
    DModulus mod = modulus[twr];
    size_t nSlots = poly_degree >> 1;

    double coeffd;
    if (coeff_id < nSlots) {
        coeffd = round(src[coeff_id].x);
    } else {
        coeffd = round(src[coeff_id - nSlots].y);
    }
...
__global__ void decompose_array_uint128_kernel(uint64_t *dst, const cuDoubleComplex *src, const DModulus *modulus,
                                               const uint32_t poly_degree) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    size_t twr = tid / poly_degree;
    size_t coeff_id = tid % poly_degree;
...
void DRNSBase::decompose_array(uint64_t *dst, const cuDoubleComplex *src, uint32_t poly_degree,
    ...
    if (max_coeff_bit_count <= 64) {
        decompose_array_uint64_kernel<<<gridDimGlb, blockDimGlb, 0, stream>>>(
                dst, src, base(), poly_degree);
    } else if (max_coeff_bit_count <= 128) {
        decompose_array_uint128_kernel<<<gridDimGlb, blockDimGlb, 0, stream>>>(
                dst, src, base(), poly_degree);
    } else {
...
__global__ void compose_array_kernel(cuDoubleComplex *dst, uint64_t *temp_prod_array, uint64_t *acc_mod_array,
...
void DRNSBase::compose_array(cuDoubleComplex *dst, const uint64_t *src, const uint64_t *upper_half_threshold,
    ...
    compose_array_kernel<<<gridDimGlb, blockDimGlb, 0, stream>>>(
