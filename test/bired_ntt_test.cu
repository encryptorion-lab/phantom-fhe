...
void test_nwt_1d(size_t log_dim, size_t batch_size) {
    ...
    for (size_t i = 0; i < batch_size; i++) {
        ...
        cudaMemcpy(twiddles + i * dim, h_ntt_table.get_from_root_powers().data(), dim * sizeof(uint64_t),
                   cudaMemcpyHostToDevice);
        cudaMemcpy(twiddles_shoup + i * dim, h_ntt_table.get_from_root_powers_shoup().data(), dim * sizeof(uint64_t),
                   cudaMemcpyHostToDevice);
        cudaMemcpy(itwiddles + i * dim, h_ntt_table.get_from_inv_root_powers().data(), dim * sizeof(uint64_t),
                   cudaMemcpyHostToDevice);
...
void test_nwt_2d(size_t log_dim, size_t batch_size) {
    ...
    for (size_t i = 0; i < batch_size; i++) {
        // generate twiddles in host
        auto h_ntt_table = NTT(log_dim, h_modulus[i]);
        d_ntt_tables.set(&modulus[i], h_ntt_table.get_from_root_powers().data(),
                         h_ntt_table.get_from_root_powers_shoup().data(), h_ntt_table.get_from_inv_root_powers().data(),
                         h_ntt_table.get_from_inv_root_powers_shoup().data(), h_ntt_table.inv_degree_modulo(),
                         h_ntt_table.inv_degree_modulo_shoup(), i);
    }
