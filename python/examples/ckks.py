...
pk = phantom.public_key(context)
sk.gen_publickey(context, pk)
rlk = phantom.relin_key(context)
sk.gen_relinkey(context, rlk)
glk = phantom.galois_key(context)
...
for i in range(1000):
    ...
    phantom.add_inplace(context, ct2, ct)
    hoisting_timer.stop()

    rotate_add_timer.start()
    ct2 = phantom.ciphertext(context)
    ct4 = phantom.ciphertext(context)
    ct8 = phantom.ciphertext(context)
    phantom.rotate_vector(context, ct, 1, glk, ct2)
    phantom.add_inplace(context, ct2, ct)
    phantom.rotate_vector(context, ct2, 2, glk, ct4)
    phantom.add_inplace(context, ct4, ct2)
    phantom.rotate_vector(context, ct4, 4, glk, ct8)
    phantom.add_inplace(context, ct8, ct4)
...
ct2 = phantom.hoisting(context, ct, glk, [1, 2, 3, 4, 5, 6, 7])
phantom.add_inplace(context, ct, ct2)
